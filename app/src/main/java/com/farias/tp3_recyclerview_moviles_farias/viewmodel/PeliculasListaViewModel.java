...
public class PeliculasListaViewModel extends ViewModel {
    ...
    public LiveData<List<PeliculaModel>> getPeliculas(){
        if(listaPeliculas == null){
            listaPeliculas = new MutableLiveData<List<PeliculaModel>>();
            cargarPeliculas();
        }
    ...
    private ArrayList<PeliculaModel> lista;

    public PeliculasListaViewModel() {
        lista = new ArrayList<>();
        lista.add(new PeliculaModel(1,"El Grinch (2000)", R.drawable.grinch,"Una criatura tiene la intención de robarse la navidad.", "Peter Seaman,Kate Soloa","James Camaron"));
        ...
        lista.add(new PeliculaModel(3,"Drácula (1958)", R.drawable.dracula,"Drácula busca vengarse de Jonathan Harker, viajando a su ciudad y atacando a la familia de su prometida. El único que puede protegerlos es el ...", "Jimmy Sangaster, Bram Stoker","Felipe Dirige"));
    ...
    public ArrayList<PeliculaModel> getLista() {
        return lista;
    }

    public void setLista(ArrayList<PeliculaModel> lista) {
        this.lista = lista;
    }

    public PeliculaModel getPeliculaXId(int id){
       PeliculaModel pelicula = null;

        for (PeliculaModel peli : lista){
            if (id == peli.getId()){
                pelicula = peli;
            }
        }
